...
const MapComponent: React.FC<MapProps> = ({
  ...
  const growingEffect = async () => {
    fetchGameState(activeAccount?.accountAddress.toString()).then((response: UserGridState[]) => {
  ...
  return (
    <div
      className="grid grid-cols-9 bg-center bg-no-repeat bg-cover shadow-custom cursor-pointer"
      style={{ backgroundImage: `url(${mapImage})` }}
    >
      {squares.map((square) => (
        <div
          key={square.id}
          className={`flip-container w-9 h-6 border border-gray text-center text-sm relative ${growingSquare === square.id ? "shake-enlarge" : ""
            }  ${currentValue === square.id ? "shake-enlarge" : ""}`}
        >
          <div className={`flip-card ${square.digs >= 100 ? "flipped" : ""}`}>
            <div
              className={`flip-front w-full h-full bg-no-repeat bg-yellow-50 ${square.digs >= 100 ? "hidden" : "block"
                }`}
              style={{
                backgroundImage: `url(${mapImage})`,
                backgroundPosition: `${(square.id % 9) * 11.1}% ${Math.floor(square.id / 9) * 16.6
                  }%`,
                backgroundSize: `calc(100% * 9) calc(100% * 6)`,
              }}
            >
              <div
                className={`absolute bottom-0 left-0 w-full h-full ${growingSquare === square.id ? "change-enlarge" : ""
                  }  ${currentValue === square.id ? "change-enlarge" : ""} ${growingSquare === square.id ? "change-color" : ""} ${currentValue === square.id ? "change-color-other" : ""}`}
                style={{
